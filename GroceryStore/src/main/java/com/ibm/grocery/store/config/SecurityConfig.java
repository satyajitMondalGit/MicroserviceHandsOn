...
import com.ibm.grocery.store.filter.JwtFilter;
...
//	private UserService userService;
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		System.out.println("Security Config 1");
		auth.getDefaultUserDetailsService();
	}

	@Bean
	public PasswordEncoder passwordEncoder() {
		System.out.println("Security Config 2");
	...
	public AuthenticationManager authenticationManagerBean() throws Exception {
		System.out.println("Security Config 3");
		return super.authenticationManagerBean();
	}

	@Override
	protected void configure(HttpSecurity http) throws Exception {
		System.out.println("Security Config 4");
		http.csrf().disable().authorizeRequests().antMatchers("/h2-console").permitAll().anyRequest().authenticated()
				.and().exceptionHandling().and().sessionManagement()
				.sessionCreationPolicy(SessionCreationPolicy.STATELESS);
		System.out.println("Security Config 5");
		http.addFilterBefore(jwtFilter, UsernamePasswordAuthenticationFilter.class);
		System.out.println("Security Config 6");
	}
}
